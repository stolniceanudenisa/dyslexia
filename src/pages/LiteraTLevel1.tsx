...
import Repeta from '../assets/sounds/alege-buline-litera-T.mp3';
import Avanseaza from '../assets/sounds/nivelul-urmator!.mp3';
import CustomToolbar from '../components/CustomToolbar';

type ButtonText = "T" | "B" | "R" | "★";

const LiteraTLevel1: React.FC<RouteComponentProps> = ({ history }) => {
    ...
    const isCorrect = (buttonText: string) => {
        if (buttonText === "T") return "success";
        if (buttonText === "B") return "tertiary";
    ...
    return (
        <IonPage>
            <IonHeader>
                <CustomToolbar title="Litera T Level 1" titleStyle="title" onPlayClick={playClickAudio} onBackClick={() => history.goBack()} />
            </IonHeader>
            <IonContent className="letter-page">
                ...
                <IonFab vertical="bottom" horizontal="end" slot="fixed">
                    <IonFabButton onClick={() => history.push('/LiteraTLevel2')} disabled={isNextLevelDisabled}>
                        <IonIcon icon={arrowForwardOutline} className="black-icon big-arrow" title="Litera T Level 2" aria-label="Next level" onMouseEnter={playHoverSoundAvanseaza} />
                    </IonFabButton>
                </IonFab>
            </IonContent>
        </IonPage>
